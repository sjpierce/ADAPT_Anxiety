...
I was running into difficulty getting that to work. I've decided to fall back to 
an ANCOVA style logistic regression modeling approach instead where I use arm 
and pretest to predict each post-test outcome. 

I need to rely on the combination of glm(), mice, broom, and emmeans 

https://www.bioinfo-scrounger.com/archives/mice-lsmean-emmeans/

https://stackoverflow.com/questions/59821087/getting-estimated-means-after-multiple-imputation-using-the-mitml-nlme-geepac


https://www.r-bloggers.com/2020/04/estimating-and-testing-glms-with-emmeans/

Given the number of positive posttest diagnoses for the different outcomes is 
sometimes between 5 and 15, I should not use more than 2 predictors (pretest and 
posttest) because one usually needs 5-9 events per parameter to get good 
estimates of regression parameters [@Hosmer-RN2752]. I would like to use age, 
gender, and race as covariates, but for most of these outcomes that is unlikely 
to work. For some of them, even having 2 predictors suggests the need for 
caution in interpreting the results. 

Code below is just to work out basic syntax with the pre-imputation data. I'll 
need to adapt it to use the imputed datasets. 

```{r}
#| label: define-G2-test

# Likelihood ratio test for model fit (applies to log-linear models fit by GLM). 
G2.test <- function(m, digits = 3) {
             G2   <- deviance(m)
             p.val <- 1 - pchisq(G2, m$df.residual)
             x     <- paste("G^2 =", round(G2, digits = digits),
                            ", df =", m$df.residual,
                            ", p =", round(p.val, digits = digits))
             print("Log-likelihood ratio statistic (residual deviance)",
                   quote = F)
             print(paste("G^2 and p-value are rounded to", digits, "digits"),
                   quote = F)
             print(x, quote = F)}
```
...
m0 <- glm(SAD_Post ~ SAD_Pre + Arm, 
          family = binomial, data = ADAPT_PreImpute, 
...
emmeans::emmeans(m0,~Arm,  trans = "response")

emmeans::emmeans(m0,~Arm,  trans = "response") %>% 
  emmeans::contrast(., method = list(c(-1, 1)), conf.int = TRUE, adjust = "none", 
                    type = "response") %>% confint()
```
