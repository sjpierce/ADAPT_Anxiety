...
Next we load the imputed data, along with the subset of the raw data that was 
used in the impoutation process. 
...
left_join(CIs_GAD, OR_GAD, by = join_by(term)) %>% 
  rename(Term = term, 
         Estimate = estimate, 
         SE = std.error,
         z = statistic,
...
```{r}
#| label: tbl-emm-GAD
#| tbl-cap: Estimated Marginal Means for GAD at Posttest

FN <- paste()
emm_GAD <- emmeans(fit_GAD, ~ Arm, regrid = "response") 
emm_GAD

plot(emm_GAD, comparisons = TRUE)
```

```{r}
#| label: tbl-emmc-GAD
#| tbl-cap: Difference Between Marginal Means for the Arms on GAD at Posttest
