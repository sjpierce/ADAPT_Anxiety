...
library(piercer)      # for all_classes(),  file_details(), git_report()
...
file_details(here("data/ADAPT_Clean.RData"))
...
file_details(here("data/ADAPT_Imputed.RData"))
...
```{r}
#| label: tbl-Sex
#| tbl-cap: Contingency Table for Sex by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Gender + Arm, addNA = TRUE, data = .) 
...
ADAPT_Clean_Data %>% 
  group_by(Arm, Gender) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = Gender, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Gender, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("Gender", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
...
```{r}
#| label: tbl-Age
#| tbl-cap: Contingency Table for Age by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Age + Arm, addNA = TRUE, data = .) 
...
ADAPT_Clean_Data %>% 
  group_by(Arm, Age) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = Age, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Age, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("Age (yrs)", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-Race
#| tbl-cap: Contingency Table for Race by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ race_cat + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  group_by(Arm, race_cat) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = race_cat, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(race_cat, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("Race", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-GAD-pre
#| tbl-cap: Contingency Table for GAD Diagnosis at Baseline by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ GAD_Pre + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(GAD_Pre = addNA(GAD_Pre)) %>% 
  group_by(Arm, GAD_Pre, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = GAD_Pre, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(GAD_Pre, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("GAD at Baseline", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-GAD-post
#| tbl-cap: Contingency Table for GAD Diagnosis at Posttest by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ GAD_Post + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(GAD_Post = addNA(GAD_Post)) %>% 
  group_by(Arm, GAD_Post, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = GAD_Post, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(GAD_Post, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("GAD at Posttest", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
CT <- ADAPT_Clean_Data %>% 
  xtabs(~ GAD_Change + Arm, addNA = TRUE, data = .) 
...
ADAPT_Clean_Data %>% 
  ...
  summarise(N = n()) %>% 
  pivot_wider(id_cols = Dx_Change, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Dx_Change, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>%
  kable(format = "html", digits = 0,
        col.names = c("GAD Change", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-Social-pre
#| tbl-cap: Contingency Table for Social Anxiety Disorder Diagnosis at Baseline by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Social_Pre + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(Social_Pre = addNA(Social_Pre)) %>% 
  group_by(Arm, Social_Pre, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = Social_Pre, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Social_Pre, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 0,
        col.names = c("Social Anxiety Disorder at Baseline", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-Social-post
#| tbl-cap: Contingency Table for Social Anxiety Disorder  Diagnosis at Posttest by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Social_Post + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(Social_Post = addNA(Social_Post)) %>% 
  group_by(Arm, Social_Post, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = Social_Post, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Social_Post, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("Social Anxiety Disorder at Posttest", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Social_Change + Arm, addNA = TRUE, data = .) 
...
ADAPT_Clean_Data %>% 
  ...
  adorn_totals(where = "row") %>%
  kable(format = "html", digits = 0,
        col.names = c("Social Anxiety Disorder Change", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-SAD-pre
#| tbl-cap: Contingency Table for SAD Diagnosis at Baseline by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ SAD_Pre + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(SAD_Pre = addNA(SAD_Pre)) %>% 
  group_by(Arm, SAD_Pre, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = SAD_Pre, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(SAD_Pre, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("SAD at Baseline", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-SAD-post
#| tbl-cap: Contingency Table for SAD Diagnosis at Posttest by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ SAD_Post + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(SAD_Post = addNA(SAD_Post)) %>% 
  group_by(Arm, SAD_Post, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = SAD_Post, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(SAD_Post, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("SAD at Posttest", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
CT <- ADAPT_Clean_Data %>% 
  xtabs(~ SAD_Change + Arm, addNA = TRUE, data = .) 
...
```{r}
#| label: tbl-Panic-pre
#| tbl-cap: Contingency Table for Panic Disorder Diagnosis at Baseline by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Panic_Pre + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(Panic_Pre = addNA(Panic_Pre)) %>% 
  group_by(Arm, Panic_Pre, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = Panic_Pre, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Panic_Pre, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("Panic Disorder at Baseline", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-Panic-post
#| tbl-cap: Contingency Table for Panic Disorder  Diagnosis at Posttest by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Panic_Post + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(Panic_Post = addNA(Panic_Post)) %>% 
  group_by(Arm, Panic_Post, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = Panic_Post, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Panic_Post, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("Panic Disorder at Posttest", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Panic_Change + Arm, addNA = TRUE, data = .) 
...
ADAPT_Clean_Data %>% 
  ...
  adorn_totals(where = "row") %>%
  kable(format = "html", digits = 0,
        col.names = c("Panic Change", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-OCD-pre
#| tbl-cap: Contingency Table for OCD Diagnosis at Baseline by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ OCD_Pre + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(OCD_Pre = addNA(OCD_Pre)) %>% 
  group_by(Arm, OCD_Pre, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = OCD_Pre, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(OCD_Pre, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("OCD at Baseline", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-OCD-post
#| tbl-cap: Contingency Table for OCD Diagnosis at Posttest by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ OCD_Post + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(OCD_Post = addNA(OCD_Post)) %>% 
  group_by(Arm, OCD_Post, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = OCD_Post, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(OCD_Post, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("OCD at Posttest", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
CT <- ADAPT_Clean_Data %>% 
  xtabs(~ OCD_Change + Arm, addNA = TRUE, data = .) 
...
```{r}
#| label: tbl-MDD-pre
#| tbl-cap: Contingency Table for MDD Diagnosis at Baseline by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ MDD_Pre + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(MDD_Pre = addNA(MDD_Pre)) %>% 
  group_by(Arm, MDD_Pre, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = MDD_Pre, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(MDD_Pre, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("MDD at Baseline", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-MDD-post
#| tbl-cap: Contingency Table for MDD Diagnosis at Posttest by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ MDD_Post + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(MDD_Post = addNA(MDD_Post)) %>% 
  group_by(Arm, MDD_Post, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = MDD_Post, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(MDD_Post, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("MDD at Posttest", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
CT <- ADAPT_Clean_Data %>% 
  xtabs(~ MDD_Change + Arm, addNA = TRUE, data = .) 
...
```{r}
#| label: tbl-Dysthymia-pre
#| tbl-cap: Contingency Table for Dysthymia Diagnosis at Baseline by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Dysthymia_Pre + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(Dysthymia_Pre = addNA(Dysthymia_Pre)) %>% 
  group_by(Arm, Dysthymia_Pre, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = Dysthymia_Pre, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Dysthymia_Pre, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("Dysthymia at Baseline", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
```{r}
#| label: tbl-Dysthymia-post
#| tbl-cap: Contingency Table for Dysthymia Diagnosis at Posttest by Arm
#| message: false
#| results: asis

CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Dysthymia_Post + Arm, addNA = TRUE, data = .) 

FN <- paste0("Two-sided Fisher's exact test for independence, p = ", 
             display_num(fisher.test(x = CT)$p.value), ".")
  
ADAPT_Clean_Data %>% 
  mutate(Dysthymia_Post = addNA(Dysthymia_Post)) %>% 
  group_by(Arm, Dysthymia_Post, .drop = FALSE) %>% 
  summarize(N = n()) %>% 
  pivot_wider(id_cols = Dysthymia_Post, names_from = Arm, values_from = N) %>% 
  mutate(Overall = ADAPT + TAU, 
         Overall.p = 100*Overall/sum(Overall),
         ADAPT.p = 100*ADAPT/sum(ADAPT),
         TAU.p = 100*TAU/sum(TAU)) %>% 
  select(Dysthymia_Post, Overall, Overall.p, ADAPT, ADAPT.p, TAU, TAU.p) %>% 
  adorn_totals(where = "row") %>% 
  kable(format = "html", digits = 1,
        col.names = c("Dysthymia at Posttest", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
CT <- ADAPT_Clean_Data %>% 
  xtabs(~ Dysthymia_Change + Arm, addNA = TRUE, data = .) 
...
ADAPT_Clean_Data %>% 
  ...
  adorn_totals(where = "row") %>%
  kable(format = "html", digits = 0,
        col.names = c("Dysthymia Change", "N", "%", "N", "%", "N", "%")) %>% 
  kable_styling() %>% 
  add_header_above(header = c(" ", "Overall" = 2, "ADAPT" = 2, "TAU" = 2)) %>% 
  add_header_above(header = c(" " = 3, "Arm" = 4)) %>% 
  footnote(kable_input = ., general = FN, footnote_as_chunk = TRUE, 
...
separation [@Heinze-RN1125]. Briefly stated, this happens when there is very 
...
Look at the raw descriptive data in @tbl-OCD-Change-Arm. They show that 
*none of the patients in the study ever worsened*. For all the other outcomes, 
at least a few people did worsen, even if it was only in one study arm. So, the
OCD outcome is unusual in that respect.

Because the model is not producing trustworthy results, I am disabling the code 
