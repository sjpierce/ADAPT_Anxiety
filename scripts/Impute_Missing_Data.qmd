...
```{r}
#| label: tbl-cvv-missingness
#| tbl-cap: Assessment of Completeness for Cases, Variables, and Values
...
cvv_missingness(ADAPT_PreImpute) %>% 
...
percent of cases that are incomplete (i.e., have any missing data). 
@tbl-cvv-missingness shows we have $N = `r N_Total`$ cases, of which 
$N_c = `r NCC`$ (`r PCC`%) are complete and $N_i = `r NIC`$ (`r PIC`%) are 
incomplete. @tbl-var-missingness shows the amount of missingness for each 
variable. 

```{r}
#| label: tbl-var-missingness

var_missingness(ADAPT_PreImpute) %>% 
  kable(format = "html", digits = 1) %>%
  kable_styling()
```
...
If `mice()` produced any logged event messages, @tbl-logged-events will have 
data in it. If not, the table content is omitted and just the table title will
show up as a placeholder. It is best if there are no logged events because those
indicate problems that would need to be solved before we can use the imputed
data.
...
@fig-traceplots-scores-pre, @fig-traceplots-scores-post, and 
@fig-traceplots-outcome-post show trace plots for batches of variables that let 
us assess whether the imputation model has converged. We need to reach 
convergence before we can rely on the results. We cannot get tracelplots for 
a variables that only had one missing value (i.e., `SCARED_Tot_Pre`, and all 
the pretest binary outcomes `GAD_Pre`, `Social_Pre`, `SAD_Pre`, `Panic_Pre`, 
`OCD_Pre`, `MDD_Pre`, `Dysthymia_Pre`) because `plot_trace()` cannot get 
convergence diagnostics (i.e., the standard deviation cannot be computed). 
...
plot_trace(ADAPT_Imputed_Data, vrb = c("DASS_Tot_Pre"), legend = FALSE)
```
...
```{r}
#| label: fig-barplots-GAD
#| fig-cap: Distributions of Observed and Imputed GAD Diagnoses at Pretest and 
#|          Posttest by Imputation Number

map(c("GAD_Pre", "GAD_Post"), ~ {
