...
imp <- mice(data = ADAPT_PreImpute, m = 5, pred = Predictors, method = "pmm", 
            maxit = 20, seed = 31766, printFlag = FALSE)
```
...
plot_trace(imp, vrb = c("DASS_Tot_Pre", "SCARED_Tot_Pre"))
```
...
plot_trace(imp, vrb = c("GAD_Pre", "Social_Pre", "SAD_Pre", "Panic_Pre",
                        "OCD_Pre", "MDD_Pre", "Dysthymia_Pre"))
```
...
plot_trace(imp, vrb = c("DASS_Tot_Post", "SCARED_Tot_Post", "CDI_Tot_Post", 
                        "PCS_Tot_Post", "FDI_Tot_Post"))
```
...
plot_trace(imp, vrb = c("GAD_Post", "Social_Post", "SAD_Post", "Panic_Post",
                        "OCD_Post", "MDD_Post", "Dysthymia_Post"))
```
...
## Diagnostics: Variable Distributions

```{r}
#| label: fig-stripplots
#| fig-cap: Imputation Strip Plot Comparing Distributionhs of Continuous 
#|          Variables for 

stripplot(x = imp, xlab = "Dataset m (0 is Unimputed, 1-5 are Imputed)")
```


```{r}
ggmice(imp, mapping = aes(x = GAD_Post)) + 
  geom_bar(fill = "white", stat = "count")
```

```{r}
ggmice(imp, mapping = aes(x = Social_Post)) + 
  geom_bar(fill = "white", stat = "count")
```

```{r}
ggmice(imp, mapping = aes(x = SAD_Post)) + 
  geom_bar(fill = "white", stat = "count")
```

```{r}
ggmice(imp, mapping = aes(x = Panic_Post)) + 
  geom_bar(fill = "white", stat = "count")
```

```{r}
ggmice(imp, mapping = aes(x = OCD_Post)) + 
  geom_bar(fill = "white", stat = "count")
```

```{r}
ggmice(imp, mapping = aes(x = MDD_Post)) + 
  geom_bar(fill = "white", stat = "count")
```

```{r}
ggmice(imp, mapping = aes(x = Dysthymia_Post)) + 
  geom_bar(fill = "white", stat = "count")
```
